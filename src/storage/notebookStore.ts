...
export type NotebookCategory = "personal-research" | "samples" | "dev-tests" | "user-requests";
...
export class NotebookStore {
  ...
  getAllNotebooks(): Notebook[] {
    ...
    return rows.map(r => ({
      ...
      category: (r.category ?? undefined) as NotebookCategory | undefined,
    }));
  }
  ...
  // ── Original .pact baseline (Index-mode reset via Abort) ────────────────────

  saveOriginalPact(notebookId: string, pact: PactExport): void {
    const db = getDb(this.extensionPath);
    db.prepare("UPDATE notebooks SET original_pact = ? WHERE id = ?").run(JSON.stringify(pact), notebookId);
  }

  getOriginalPact(notebookId: string): PactExport | null {
    const db = getDb(this.extensionPath);
    const row = db
      .prepare("SELECT original_pact FROM notebooks WHERE id = ?")
      .get(notebookId) as { original_pact: string | null } | undefined;
    if (!row?.original_pact) return null;
  ...
  exportNotebook(notebookId: string): PactExport | null {
    ...
    const xmState = this.getXmState(notebookId);
    ...
    return {
      ...
      notebook: {
        ...
        ...(notebookRow.category ? { category: notebookRow.category as NotebookCategory } : {}),
      },
  ...
  importNotebook(data: PactExport): Notebook {
    ...
    // category simply carries through whatever was embedded in the file —
    // never assigned or guessed here; a notebook's category is set once at
    // creation and travels with it through every future export/import.
    const category = data.notebook.category;

    db.prepare(`
      INSERT INTO notebooks (id, name, is_system, created_at, system_prompt, execution_mode, category)
      VALUES (?, ?, 0, ?, ?, ?, ?)
    `).run(notebookId, data.notebook.name, createdAt, data.notebook.systemPrompt ?? null, executionMode, category ?? null);
    ...
    if (data.xmState) {
      this.saveXmState(notebookId, {
        ...data.xmState,
        discussionId: discussionIdMap[data.xmState.discussionId] ?? data.xmState.discussionId,
        activeCellId: cellIdMap[data.xmState.activeCellId] ?? data.xmState.activeCellId,
      });
    }

    return { id: notebookId, name: data.notebook.name, isSystem: false, createdAt, executionMode, category };
  }
